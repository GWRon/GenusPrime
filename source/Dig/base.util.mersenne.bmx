...
'as the costs of the no-repetition-check are directly dependend of the
'amount it is only useful for small amount values.
global rrarr:object[] = new Object[500]
for local i:int = 0 until rrarr.length
	rrarr[i] = object(string(i))
next
Function RandRangeArray3:object[](amount:int = 1)
	amount = min(amount, rrarr.length)

	Local all:int = rrarr.length
	Local last:Int = all - 1

	Local result:object[] = new object[amount]
	For Local c:Int = 0 Until amount
		Local index:Int = RandRange(0, last)
		result[c] = rrarr[index]

		rrarr[index] = rrarr[last]
		last :- 1
	Next
	return result
End Function
